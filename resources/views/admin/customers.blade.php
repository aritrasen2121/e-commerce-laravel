<x-admin-layout>   
    ...
    <div class="bg-gray-800 rounded-lg border-gray-300 dark:border-gray-600 h-[41rem] p-5 ">
            <div
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg h-[33rem]">
                @if (@isset($users))
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                              ...
                              <td class="px-6 py-4">
                                  <form action="{{ route('admin.customers.change-role',$user->id) }}" method="POST">
                                    @csrf
                                    @method('put')
                                    <button class="hover:text-blue-600 hover:underline">{{ $user->type == 'admin'? 'make user' : 'make admin'}}</button>
                                  </form>
                              </td>
                        <td class="px-6 py-4 font-semibold">
                            <form action="{{ route('admin.customers.change-status',$user->id) }}" method="POST">
                                @csrf
                                @method('put')
                                @if ($user->status=='active')
                                    <button class="text-red-600 underline mr-2" type="submit">Inactive</button>                                
                                @elseif ($user->status=='inactive')
                                    <button class="text-blue-600 underline" type='submit'>Active</button>
